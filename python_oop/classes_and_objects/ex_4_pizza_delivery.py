...
class PizzaDelivery:
    ...
    def make_order(self) -> str:
        self.ordered = True
        result_ingredients = ', '.join([f"{key}: {value}" for key, value in self.ingredients.items()])
        return f"You've ordered pizza {self.name} prepared with {result_ingredients} and the price will be {self.price}lv."
